...
namespace Games.TLBB.Manager.IO
{
    class EditorResourceLoader : ResourceLoader
    {
        ...
        public override Byte[] LoadByteBuffer(string mainPath, Int32 offset, Int32 length, bool lockFile)
        {
            if (isEditor)
            {
                FileStream fin = File.OpenRead(dataPath + "/../StreamingAssets/" + mainPath);
                if( fin != null)
                {
            ...
            {
                return ResourceLoader.m_StreamingAssetsLoader.Load(mainPath, offset, length);
            }
        }
        public override Byte[] LoadByteBuffer(string mainPath, bool lockFile)
        {
            if (isEditor)
            {
                return File.ReadAllBytes(dataPath + "/../StreamingAssets/" + mainPath);
            }
            else
            {
        ...
        {
            return CusEncoding.EncodingUtil.FileByteToLocal(LoadByteBuffer(mainPath,false));
        }
        public override String LoadTextString(string mainPath, bool decode)
        {
            return new String(System.Text.Encoding.UTF8.GetChars(CusEncoding.EncodingUtil.FileByteToLocal(LoadByteBuffer(mainPath, false))));
        }
        ...
        {
            BaseLogSystem.internal_Error("暂没实现 Error!");
        }
        ...
        public override void DestroyResource(System.Object obj)
        {
            ...
            if (uObj != null)
            {
                ...
                {
//                     foreach (System.Object o6 in m_vResourceLoadedGameObjectset)
//                     {
//                         System.Object wwwQ = o6;
//                         int www = 100;
// 
//                     }
...
//         public override bool ReleaseTSRData(string path)
//         {
//             return true;
//       
//         }
...
//                 m_LoadedResourceDataDic.Remove(index);
            ...
            {
                BaseLogSystem.internal_Error("ResourceData Error!!");
            }
        }
        ...
        //LoadAssetAtPath
        private MethodInfo methodLoad = null;
        ...
        public override UnityEngine.Object LoadResource(IResConfig rc)
        {
            ...
            {
                case (byte)(PATH_TYPE.PATH_TYPE_GAMEMAIN):
                    {
                        if (Application.isEditor)
                        {
                            if (methodLoad == null)
                            {
                                System.Type t = System.Type.GetType("UnityEditor.AssetDatabase,UnityEditor");
                                if (t != null)
                                {
                                    methodLoad = t.GetMethod("LoadAssetAtPath", new Type[] { typeof(System.String), typeof(System.Type) });
                                }
                            }

                            if (methodLoad == null)
                            {
                                BaseLogSystem.internal_Error("LoadResource GameMainPath GetMethod LoadAssetAtPath Failed!!");
                                return null;
                            }

                            UnityEngine.Object obj = (UnityEngine.Object)methodLoad.Invoke(null, new object[] { config.MainPath, typeof(UnityEngine.Object) });
                        ...
                        {
                            BaseLogSystem.internal_Error("LoadResource Cannot load GameMainPath without Editor!");
                            return null;
                        }                   
                    }
                ...
                case (byte)(PATH_TYPE.PATH_TYPE_STREAMINGASSETS):
                    {
                        BaseLogSystem.internal_Error("LoadResource Cannot load streamingAssetsPath!");
                        return null;
                    }
                case (byte)(PATH_TYPE.PATH_TYPE_PERSISTENT):
                    {
                        BaseLogSystem.internal_Error("LoadResource Cannot load persistentDataPath!");
                        return null;
                    }
                default:
                    BaseLogSystem.internal_Error("IResConfig.PathType is Invalid！{0} ", config.MainPathType);
                    break;
            }
            return null;
        }
        ...
        public override String LoadTextResource(IResConfig rc)
        {
            ...
            switch (config.MainPathType)
            {
                ...
                case (byte)(PATH_TYPE.PATH_TYPE_GAMEMAIN):
                    {
                        BaseLogSystem.internal_Error("LoadResource Cannot load Resources!");
                        return null;
                    }
                case (byte)(PATH_TYPE.PATH_TYPE_STREAMINGASSETS):
                    {
        ...
        {
            return LoadByteBuffer(fileName,false);
        }
        ...
        protected void TickRemoveResource()
        {
            ...
            while (iter.MoveNext())
            {
                ...
                if (bRet == false)
                {
                    BaseLogSystem.internal_Error("RemoveResource {0} Not Find!!", index);
                }
            }
            m_StopResourceSet.Clear();
        }
            ...
            {
                {
                    if (m_LoadingDontDestroyResouce.resObject)
                    {
                        ...
                        if (m_LoadingDontDestroyResouce.isInstantiate)
                        {
                            ...
                            {
                                BaseLogSystem.internal_Error("资源类型不能实例化!!!" + m_LoadingDontDestroyResouce.resObject.ToString());
                            }
                ...
                if (enu.MoveNext())
                {
                    ...
                    if (m_LoadingDontDestroyResouce.configArray != null)
                    {
                        for(Int32 i=0;i<m_LoadingDontDestroyResouce.configArray.Length;++i)
                        {
                            ...
                            {
                                BaseLogSystem.internal_Debug("TickDontDestroyOnLoad Failed!! resPath = {0}  resObjectArray == null", m_LoadingDontDestroyResouce.configArray[i].MainPath);
                    ...
                    else
                    {
                        ...
                        {
                            BaseLogSystem.internal_Debug("TickDontDestroyOnLoad Failed!! resPath = {0}  request == null", m_LoadingDontDestroyResouce.config.MainPath);
                            RecycleResourceData(m_LoadingDontDestroyResouce);
                            m_LoadingDontDestroyResouce = null;
                        }
                    }
                        ...
                        else
                        {
                            if (rData.configArray != null)
                            {
                                ...
                                for (int i = 0; i < rData.configArray.Length; ++i)
                                {
                                    ...
                                    {
                                        BaseLogSystem.internal_Error("Load Resource Failed!! resPath = {0}", rData.configArray[i].MainPath);
                                        RecycleResourceData(rData);
                                        bError = true;
                                        break;
                                    }
                            ...
                            else
                            {
                                ...
                                if (rData.resObject == null)
                                {
                                    BaseLogSystem.internal_Error("Load Resource Failed!! resPath = {0}", rData.config.MainPath);
                                    RecycleResourceData(rData);
                                }
                                else
                                {
...
#region ResourcePriority_SynText
            ...
            for (i = dataList.Count - 1; i >= 0; --i)
            {
                ...
                if( rData.resObject)
                {
                    ...
                    {
                        BaseLogSystem.internal_Error("ResourcePriority_SynText Can not Load Other Type!!! {0}", rData.config.MainPath);
                        RecycleResourceData(rData);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (int j = 0; j < rData.resObjectArray.Length;++j)
                        {
                            ...
                            {
                                BaseLogSystem.internal_Error("ResourcePriority_SynText Can not Load Other Type!!! {0}", rData.resObjectArray);
                                RecycleResourceData(rData);
                                bError = true;
                                break;
                            }
