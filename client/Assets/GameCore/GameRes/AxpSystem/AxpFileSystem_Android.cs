...
namespace AxpTools
{
    public class AxpFileSystem : IAxpSystem
    {
        ...
        //C++读取 StreamingAssets下资源
        [DllImport("native-lib", EntryPoint = "Java_com_example_woonam_myapp_MyNDK_InitialDLL")]
        public static extern int Java_com_example_woonam_myapp_MyNDK_InitialDLL();

        [DllImport("native-lib", EntryPoint = "Android_SeekReadAssets")]
        public static extern IntPtr Android_SeekReadAssets(string filename, ref int size, ref int offset,bool lockFile);

        [DllImport("native-lib", EntryPoint = "Android_SetReadFileCount")]
        public static extern void Android_SetReadFileCount(ref int nFileCount);

        [DllImport("native-lib", EntryPoint = "Android_GetReadFileCount")]
        public static extern IntPtr Android_GetReadFileCount();

        [DllImport("native-lib", EntryPoint = "Android_MemoryInfo")]
        public static extern int Android_MemoryInfo(string filename);

        private AndroidJavaClass _helper;
        public AndroidJavaClass helper
        {
            get
            {
                if (_helper != null) return _helper;
                _helper = new AndroidJavaClass("woonam.unity.assetsloader.UnityAndroidAssetsLoader");
        ...
        public AndroidJavaObject ActivityJO
        {
            get
            {
                if (activity_ != null)
        ...
        public override bool Initial()
        {
            ...
            if (Java_com_example_woonam_myapp_MyNDK_InitialDLL() == 0)
            {
                Games.TLBB.Log.LogSystem.Info("Initial MyLibrary.so Failed.");
                return false;
            }
            else
            {
                int nFileCount = 0;
                Android_SetReadFileCount(ref nFileCount);
                Games.TLBB.Log.LogSystem.Info("Initial MyLibrary.so Successed.");
            }
#endif
...
//             }
        ...
        {
            return Android_MemoryInfo(fileName);
        }
        ...
        public override byte[] openFileByBuffer(string strFileName, out string axpFileName, out AxpFilePath axpFilePath, out uint dataSize, out uint offset)
        {
            ...
            if (axp != null && blockNodeID != -1)
            {
                if(axp.axpFilePath == AxpFilePath.StreamingAssetsPath)
                {
#if UNITY_ANDROID && (!UNITY_EDITOR)
                    if(axp.openFileByBuffer_ByCPlusPlus(blockNodeID, out dataSize, out offset))
                    {
                        int t_dataSize = (int)dataSize;
                        int t_offset = (int)offset;
                        IntPtr t_Ptr = Android_SeekReadAssets(  axp.m_strFileName, ref t_dataSize,ref t_offset,false);
                        if(t_Ptr == IntPtr.Zero)
                        {
                            Games.TLBB.Log.LogSystem.Error("读取Axp:"+ axp.m_strFileName +"   " + axp.m_strPathFileName +"获取文件:" + strFileName + "失败" + "文件大小:" + t_dataSize + "偏移:" + t_offset);
                            return retByte;
                        }

                        retByte = new byte[t_dataSize];
                        Marshal.Copy(t_Ptr, retByte, 0, t_dataSize);
                        Marshal.FreeHGlobal(t_Ptr);

                        
                    }
                    else
                    {
                        Games.TLBB.Log.LogSystem.Error("读取Axp:"+ axp.m_strFileName +"   " + axp.m_strPathFileName +"获取文件:" + strFileName + "失败." + "BLOCKNODEID :" + blockNodeID + "文件大小:" + dataSize + "偏移:" + offset);
                        return retByte;
                    }
#else
                    return null;
        ...
        public override byte[] openFileByBuffer(string strFileName, uint offset, uint dataSize, bool lockFile)
        {
            ...
            if (axp != null && blockNodeID != -1)
            {
                if (axp.axpFilePath == AxpFilePath.StreamingAssetsPath)
                {
                    ...
                    uint baseOffset = 0;
                    uint baseSize = 0;
                    if(axp.openFileByBuffer_ByCPlusPlus(blockNodeID, out baseSize, out baseOffset))
                    {
                        if( dataSize + offset > baseSize)
                            return null;

                        int t_dataSize = (int)dataSize;
                        int t_offset = (int)(offset+baseOffset);
                        IntPtr t_Ptr = Android_SeekReadAssets(  axp.m_strFileName, ref t_dataSize,ref t_offset,lockFile);
                        if(t_Ptr == IntPtr.Zero)
                        {
                            Games.TLBB.Log.LogSystem.Error("读取Axp:"+ axp.m_strFileName +"   " + axp.m_strPathFileName +"获取文件:" + strFileName + "失败" + "文件大小:" + t_dataSize + "偏移:" + t_offset);
                            return retByte;
        ...
        private bool insertFile(uint nMaxAxpFileSize, List<string> fileKeys, List<string> files)
        {
            ...
            if (lastAxpFile == null || lastAxpFile.isReadOnly)
            {
                lastAxpFile = lastAxpFile = CreatNewAxp(m_AxpMap.gameVersion, m_AxpMap.resFirstVersion, m_AxpMap.resSecondVersion, m_AxpMap.resThirdVersion);
                if (lastAxpFile == null)
                    return false;
            }
            uint t_FileCount = lastAxpFile.fileCount;
        ...
        private bool savePakFile(AxpFile axp, List<string> t_AllFiles)
        {
            ...
            foreach (string item in t_AllFiles)
            {
                ...
                //Games.TLBB.Log.LogSystem.Info("@Already insert axp file name:{0}, file name : {1}", axp.m_strFileName, item);
            }
            return true;
        }
