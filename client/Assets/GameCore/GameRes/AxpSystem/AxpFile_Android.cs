    ...
    public class AxpFile
    {
        ...
        public bool openPakFile(string strPackFileName, AxpFilePath path)
        {
           ...
           Games.TLBB.Log.LogSystem.Info("Axp GameVersion : {0} , ResFirstVersion : {1} , ResSecondVersion : {2},  ResThirdVersion: {3}", m_fileHead.gameversion, m_fileHead.resfirstversion, m_fileHead.ressecondversion, m_fileHead.resthirdversion);
            ...
            if (path == AxpFilePath.StreamingAssetsPath)
            {
                m_fileStream.Close();
                m_fileStream = null;
            }
        ...
        public bool openFile(int blockNodeIndex, out uint dataSize, out uint offSet)
        {
            ...
            //Games.TLBB.Log.LogSystem.Error("Open Axp File :{0} dataSize: {1} offset: {2}", m_strPathFileName, dataSize.ToString(), offSet.ToString());
            return true;
        }
