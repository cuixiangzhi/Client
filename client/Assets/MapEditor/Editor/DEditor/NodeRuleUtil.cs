...
public class NodeRuleUtil {
    ...
    //foreach (var node in new_data.nodes.emptyNodes)
    //{
    //    if (node.parents.Count != 0)
    //    {
    //        bool from_condition = false;
    //        for (int j = 0; j < node.parents.Count; j++)
    //        {
    //            TriggerNodeData _parent = GetNode(node.parents[j], new_data);
    //            if (_parent.NodeType == (int)BaseNode.NodeType.Condition)
    //            {
    //                from_condition = true;
    //                break;
    //            }
    ...
    //        if (from_condition)
    //        {
    //            List<TriggerConditionData> team = GetTeam(node.id, new_data);
    //            for (int i = 0; i < team.Count; i++)
    //            {
    //                team[i].node.secureDoor = node.id;
    //            }

    //            node.NodeType = (int)BaseNode.NodeType.Security;
    //        }
    //    }
    //}

    //static List<TriggerConditionData> GetTeam(int nodeID, NewTriggerInfo trigger)
    //{
    //    List<TriggerConditionData> conditions = new List<TriggerConditionData>();
    //    TriggerConditionData condition = GetCondition(nodeID, trigger);
    //    conditions.Add(condition);
    //    if (condition.node.NodeType != (int)BaseNode.NodeType.Empty && condition.node.NodeType != (int)BaseNode.NodeType.Security)
    //    {
    //        for (int i = 0; i < condition.node.parents.Count; i++)
    //        {
    //            List<TriggerConditionData> tmp = GetTeam(condition.node.parents[i], trigger);
    //            foreach (TriggerConditionData con in tmp)
    //            {
