...
public class UIWrapContent : MonoBehaviour
{
	...
	protected virtual void ResetChildPositions ()
	{
		for (int i = 0, imax = mChildren.Count; i < imax; ++i)
		{
            ...
            int row = i / itemCountPerLine, col = i % itemCountPerLine;
            t.localPosition = mHorizontal ? new Vector3(row * itemWidth, -col * itemHeight, 0f) : new Vector3(col * itemWidth,-row * itemHeight , 0f);
            UpdateItem(t, i);
        }
	}
	/// <summary>
	...
	{
		float extents = (mHorizontal ? itemWidth : itemHeight) * mChildren.Count / itemCountPerLine * 0.5f;
		...
		if (mHorizontal)
		{
			...
			for (int i = 0, imax = mChildren.Count; i < imax; ++i)
			{
                ...
                else if(distance < -extents)
				{
					...
					int realIndex = Mathf.RoundToInt(pos.x / itemWidth) * itemCountPerLine + Mathf.RoundToInt(-pos.y / itemHeight);
		...
		else
		{
			...
			for (int i = 0, imax = mChildren.Count; i < imax; ++i)
			{
                ...
                else if(distance < -extents)
				{
					...
					int realIndex = Mathf.RoundToInt(-pos.y / itemHeight) * itemCountPerLine + Mathf.RoundToInt(pos.x / itemWidth);
